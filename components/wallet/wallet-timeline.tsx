...
import type { WalletHistoryRow } from "@/lib/wallet-history";
import { formatPrice } from "@/lib/product-utils";
import { formatWalletDeltaSar } from "@/lib/wallet-history";
...
export function WalletTimeline({ rows }: { rows: WalletHistoryRow[] }) {
  if (rows.length === 0) {
    return (
      <Card className="border-dashed border-sky-200/80 bg-gradient-to-br from-sky-50/40 via-white to-emerald-50/20">
        <CardContent className="flex flex-col items-center gap-4 py-16 text-center">
          <div className="flex size-16 items-center justify-center rounded-2xl bg-emerald-500/10 text-emerald-700 ring-4 ring-emerald-500/5">
            <Wallet className="size-8" aria-hidden />
          </div>
          <div className="max-w-sm space-y-2">
            <p className="text-lg font-bold text-sky-950">لا يوجد سجل محفظة بعد</p>
            <p className="text-sm leading-relaxed text-muted-foreground">
              عند إضافة رصيد أو استخدامه في الطلبات ستظهر الحركات هنا بالتفصيل والعملة.
            </p>
          </div>
          <Link
            href={ROUTES.shop}
  ...
  return (
    <ul className="grid gap-3" aria-label="سجل المحفظة">
      {rows.map((row) => {
          ...
          <li key={row.key}>
            <Card
              className={cn(
                ...
                positive &&
                  "border-s-4 border-s-emerald-500/35 bg-gradient-to-l from-emerald-50/35 to-transparent",
                ...
                <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-t border-sky-100/80 pt-3 sm:flex-col sm:items-end sm:border-t-0 sm:pt-0 sm:text-end">
                  {row.delta != null ? (
                    <span
                      ...
                      dir="ltr"
                    >
                      {formatWalletDeltaSar(row.delta)}
                    </span>
                  ) : (
                  ...
                  {row.balanceAfter != null ? (
                    <span className="text-[11px] font-medium text-muted-foreground tabular-nums" dir="ltr">
                      الرصيد بعدها: {formatPrice(row.balanceAfter)}
                    </span>
                  ) : null}
                </div>
              </CardContent>
